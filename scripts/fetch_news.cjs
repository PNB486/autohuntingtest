...
async function run() {
  ...
  for (let item of allNews) {
    ...
    const posKeywords = ['양산', '수주', 'HBM4', '상승', '호재', '호황', '돌입', '성공', '돌파'];
    const negKeywords = ['위험', '전쟁', '하락', '악재', '고환율', '금리인하 지연', '분쟁', '캐즘', '갈등', '금리인하지연'];
    
    let isPos = posKeywords.some(kw => textToSearch.includes(kw));
    let isNeg = negKeywords.some(kw => textToSearch.includes(kw));
    
    let tagHtml = '';
    if (isPos) {
      tagHtml = `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 mb-2">🟢 호재 | 수주/호재</span>`;
    } else if (isNeg) {
      tagHtml = `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-500/10 text-rose-400 border border-rose-500/20 mb-2">🔴 악재 | 리스크</span>`;
    } else {
      tagHtml = `<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-500/10 text-slate-400 border border-slate-500/20 mb-2">🟡 중립 | ${item.category}</span>`;
    }
    ...
    <div class="card rounded-2xl overflow-hidden shadow-xl" onclick="window.open('${item.link}', '_blank')">
        ...
        <div class="p-5">
            <div class="flex justify-between items-center mb-2">
                <span class="text-xs font-semibold text-sky-400 uppercase tracking-wider">${item.category}</span>
                ${tagHtml}
            </div>
