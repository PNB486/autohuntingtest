...
def validate_file(file_path):
    ...
    try:
        ...
        # 5. Check for Chart.js Mandates
        if '<canvas' in content:
            # Check for fixed-height container
            if 'relative h-[' not in content and 'style="height:' not in content and 'style="position: relative; height:' not in content:
                print(f"  [ERROR] Canvas found without fixed-height relative container in {file_path}")
                return False

            # Check for Chart.js options
            if 'responsive: true' not in content or 'maintainAspectRatio: false' not in content:
                print(f"  [ERROR] Chart.js options missing responsive:true or maintainAspectRatio:false in {file_path}")
                return False

            # Check for standardized CDN
            if 'cdn.jsdelivr.net/npm/chart.js@4.4.1' not in content:
                print(f"  [ERROR] Non-standard Chart.js CDN detected in {file_path}")
                return False

            # Check for DOMContentLoaded wrapper
            if 'DOMContentLoaded' not in content:
                print(f"  [ERROR] Chart initialization not wrapped in DOMContentLoaded in {file_path}")
                return False

        # 4. Check for gradient shorthand issue
        # (Should use background-image for .hero-gradient)
